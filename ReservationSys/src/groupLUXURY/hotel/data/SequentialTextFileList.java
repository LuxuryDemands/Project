...
public class SequentialTextFileList implements ListPersistenceObject {
	 ...
	 * 
	 * @param roomFilename
	 *            Text file with sorted Rooms
	 * @param customerFilename
	 *            Text file with sorted Customers
	 * @param reservationFilename
	 *            Text file with sorted Reservations
	 */
	public SequentialTextFileList(String roomFilename, String customerFilename, String reservationFilename) {
	...
	public List<Room> getRoomDatabase() {
		Room[] rooms;
		try {
			rooms = HotelFileLoader.getRoomListFromSequentialFile(roomFilename);
		} catch (IOException e) {
	...
	public List<Customer> getCustomerDatabase() {
		Customer[] custs;
		try {
			custs = HotelFileLoader.getCustomerListFromSequentialFile(customerFilename);
		} catch (IOException e) {
	...
	public List<Reservation> getReservationDatabase() {
		...
		try {
			rooms = HotelFileLoader.getRoomListFromSequentialFile(roomFilename);
			custs = HotelFileLoader.getCustomerListFromSequentialFile(customerFilename);
			reservs = HotelFileLoader.getReservationListFromSequentialFile(reservationFilename, custs, rooms);
		} catch (IOException e) {
	...
	public void saveReservationDatabase(List<Reservation> reservs) throws IOException {
		// For now we’ll use the existing saveListToTextFile utility method.
		Reservation[] resArray = reservs.toArray(new Reservation[reservs.size()]);
